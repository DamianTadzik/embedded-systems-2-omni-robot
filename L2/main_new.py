...
class FilterSetup:
    ...
    def step(self, measurement_angle, measurement_dist, new_detection_flag, vx, vy, omega):
        if self.filter_choice == 'ALPHA_BETA':
            ...
            est_dist = self.dist_filter.step(measurement_dist, new_detection_flag, vx)
        else:
            raise FilterConfigError("Improper filter choice")
...
class Follower:
    def __init__(self, sp_angle, sp_distance, controller_choice, filter_choice):
        self.detector_active = False
        self.last_detection_time = 0
        self.controller = ControlSetup(controller_choice, sp_angle, sp_distance)
